...
export type ScoreField = keyof TeamScore;
...
export const createRaid = (name: string, teams: Team[]): Raid => ({
  id: crypto.randomUUID(),
  name,
  scores: Object.fromEntries(teams.map((team) => [team.id, { ...EMPTY_SCORE }])),
});
...
export const getScoreTotal = (
  ...
  score.extractedLoot +
  score.kills * scoringValues.killValue +
...
export const parseState = (snapshot: string): AppState => {
  try {
    ...
    return {
      ...
      raids: Array.isArray(parsedState.raids) ? parsedState.raids : [],
      scoringValues: {
